...
def ShowmeMyPets(USER):
       ...
       if(len(Pets)==0):
              ...
              my_game = ttk.Treeview(tktk,yscrollcommand=Pets_scroll.set, xscrollcommand =Pets_scroll.set)
...
def popupPricemsg(f,date1,date2,tkvar):
       if tkvar=='None':
              ...
              if(d2<d1):
                     popup = tk.Toplevel()
                     popup.title("!")
                     label = tk.Label(popup, text='Your check out date must be after your check in date !')  # Can add a font arg here
                     ...
                     popup = tk.Toplevel()
                     popup.title("!")
                     label = tk.Label(popup, text='Total price : {0}'.format(price))  # Can add a font arg here
                     label.pack(side="top", fill="x", pady=10)
...
def Reservation(USER):
       ...
       Button(tktk, command=lambda :popupPricemsg(ReservePage,cal1.get_date(),cal2.get_date(),tkvar.get()), text='Submit', width=20, bg='brown', fg='white').place(x=180, y=380)
